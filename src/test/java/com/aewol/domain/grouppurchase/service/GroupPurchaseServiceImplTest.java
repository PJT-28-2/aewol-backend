...
class GroupPurchaseServiceImplTest {
    ...
    @Test
    @DisplayName("마감 미달 PAID 참여자가 여러 명이면 전원 환불하고 처리 건수를 반환한다")
    void should_refundAllCandidates_when_multipleExpiredParticipantsExist() {
        GroupPurchaseServiceImpl service = service();
        Map<String, Object> candidate1 = participantRow(101L, "1", "member-1", 2);
        candidate1.put("payment_status", "PAID");
        candidate1.put("paid_amount", new BigDecimal("50000"));
        Map<String, Object> candidate2 = participantRow(102L, "2", "member-2", 1);
        candidate2.put("payment_status", "PAID");
        candidate2.put("paid_amount", new BigDecimal("25000"));
        when(groupPurchaseMapper.findExpiredUnfulfilledPaidParticipants())
                .thenReturn(List.of(candidate1, candidate2));
        when(groupPurchaseMapper.cancelParticipant(eq("1"), eq("member-1"), any())).thenReturn(1);
        when(groupPurchaseMapper.cancelParticipant(eq("2"), eq("member-2"), any())).thenReturn(1);
        when(groupPurchaseMapper.findById("1")).thenReturn(savedRow());
        Map<String, Object> gp2 = savedRow();
        gp2.put("gp_id", "2");
        when(groupPurchaseMapper.findById("2")).thenReturn(gp2);

        Map<String, Object> wallet1 = new HashMap<>();
        wallet1.put("wallet_id", "wallet-1");
        wallet1.put("balance", new BigDecimal("100000"));
        Map<String, Object> wallet2 = new HashMap<>();
        wallet2.put("wallet_id", "wallet-2");
        wallet2.put("balance", new BigDecimal("30000"));
        ...
        int refundedCount = service.processExpiredRefunds();

        assertEquals(2, refundedCount);
        verify(groupPurchaseMapper).decreaseQuantityForExpired("1", 2);
        verify(groupPurchaseMapper).decreaseQuantityForExpired("2", 1);
        verify(walletMapper).addBalance("wallet-1", new BigDecimal("50000"));
        verify(walletMapper).addBalance("wallet-2", new BigDecimal("25000"));
        verify(transactionMapper, times(2)).insert(anyMap());
    }

    @Test
    @DisplayName("후보 조회 결과가 없으면 아무 것도 처리하지 않고 0을 반환한다")
    void should_returnZero_when_noExpiredCandidates() {
        GroupPurchaseServiceImpl service = service();
        when(groupPurchaseMapper.findExpiredUnfulfilledPaidParticipants()).thenReturn(List.of());
    ...
    @Test
    @DisplayName("이미 leave/작성자취소 등으로 처리되어 cancelParticipant의 영향 행이 0인 후보는 건너뛴다")
    void should_skipCandidate_when_cancelParticipantAffectsNoRows() {
        GroupPurchaseServiceImpl service = service();
        Map<String, Object> candidate = participantRow(101L, "1", "member-1", 2);
        candidate.put("payment_status", "PAID");
        candidate.put("paid_amount", new BigDecimal("50000"));
        when(groupPurchaseMapper.findExpiredUnfulfilledPaidParticipants()).thenReturn(List.of(candidate));
        // 이 배치와 별개로 leave()나 작성자 취소가 먼저 CANCELLED로 바꿔놓은 상황을 재현한다.
        when(groupPurchaseMapper.cancelParticipant(eq("1"), eq("member-1"), any())).thenReturn(0);
