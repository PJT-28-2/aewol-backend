...
public class InsuranceSimulationServiceImpl implements InsuranceSimulationService {
    ...
    private RecommendedProductResponse toRecommendedProduct(Map<String, Object> product,
        ...
        return RecommendedProductResponse.builder()
                ...
                .deductibleApplied(isDeductibleApplied(referenceTier))
    ...
    /**
     * 자기부담금 분기표(계획서 Decision 5)를 구현한다.
     *
     * <p>{@code n}(연 진료 횟수) 출처가 아직 미확정({@link #CLAIM_COUNT_SOURCE_CONFIRMED}=false)이므로
     * 현재는 아래 두 행만 실질적으로 동작한다:</p>
     * <ul>
     *   <li>{@code deductible_krw}가 NULL → 자기부담금 미반영, {@code C × rate/100} 그대로 적용
     *       (자기부담금 컬럼이 아직 백필되지 않은 현재 시드에서는 항상 이 분기를 탄다 —
     *       기존 골든 테스트 수치가 그대로 유지되는 이유)</li>
     *   <li>{@code deductible_basis='PER_YEAR'} → n과 무관하게 {@code max(0, C−d) × rate/100} 적용</li>
     *   <li>{@code deductible_basis='PER_VISIT'} & n 미확보 → 해당 상품만 자기부담금 미반영</li>
     * </ul>
     * <p>{@code n} 출처가 확정되면 {@link #CLAIM_COUNT_SOURCE_CONFIRMED}를 true로 바꿔
     * PER_VISIT 확보 분기({@code max(0, C−d×n) × rate/100})가 활성화되도록 한다. TODO(S2).</p>
     *
     ...
     *
     * <p>{@code deductibleKrw}가 채워져 있어도 기준(건당/연간)을 모르면 반영하지 않으므로
     * (아래 {@link #calculateAnnualReimbursement}의 가드), 금액 유무만으로는 화면이
     * "자기부담금 미반영" 고지를 정확히 낼 수 없다. 그 판정을 서버가 단일 원천으로
     * 내려준다 — 프론트가 조건을 재구성하면 산식이 두 곳에 생긴다(Principle 2).</p>
     */
    private Boolean isDeductibleApplied(Map<String, Object> referenceTier) {
        if (referenceTier == null || referenceTier.get("deductible_krw") == null) {
            return false;
        }
        ...
        return !("PER_VISIT".equals(basis) && !CLAIM_COUNT_SOURCE_CONFIRMED);
    }
    ...
    private BigDecimal calculateAnnualReimbursement(BigDecimal annualCost, int reimbursementRatePct,
        ...
        BigDecimal deductible = referenceTier == null ? null : (BigDecimal) referenceTier.get("deductible_krw");
        String basis = referenceTier == null ? null : (String) referenceTier.get("deductible_basis");
        ...
        if (basis == null) {
            // 기준(건당/연간) 미확인 행: 어느 쪽으로도 가정하지 않고 미반영한다.
            // 이 가드가 없으면 아래 삼항 연산자의 else로 떨어져 PER_YEAR로 취급되는데,
            // 실제 약관이 건당(PER_VISIT)이면 차감액이 n배 작아져 보장금이 과대 계산된다
            // (메리츠 자부담 3만원 기준: 연 1회 차감 33,850원 과대). 모르는 값을 유리한
            // 쪽으로 가정하지 않는다 — 계획서 Principle 1.
            return annualCost.multiply(rate);
        }
        ...
        BigDecimal deductionAmount = ("PER_VISIT".equals(basis))
                ? deductible.multiply(BigDecimal.valueOf(ANNUAL_CLAIM_COUNT))
                : deductible;

        if ("AFTER_RATE".equals(order)) {
        ...
        // 기본(BEFORE_RATE, 선차감): max(0, C-d[×n]) × rate/100
    ...
    private void persistSimulation(String petId, List<String> historyCodes, InsuranceAdvice advice,
        ...
        simulation.put("assumptionSource", ASSUMPTION_SOURCE);
