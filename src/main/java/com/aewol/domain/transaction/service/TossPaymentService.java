...
/**
 * Toss 결제 승인을 오케스트레이션하는 비트랜잭셔널 서비스.
 *
 ...
 * 동시에 10건의 결제만 몰려도 로그인·지갑조회 등 결제와 무관한 모든 요청이 커넥션 획득
 * 타임아웃(30초)까지 함께 멈춘다. 원장 기록만 {@link TransactionService#recordExternalPayment}
 * 의 독립된 짧은 트랜잭션으로 처리하고, Toss HTTP 호출은 항상 트랜잭션 밖에서 실행한다.
 * 나중에 누군가 "편의상" {@code @Transactional}을 이 클래스에 붙이지 않도록 주의할 것.
 */
@Slf4j
@Service
...
public class TossPaymentService {
    ...
    public TransactionResponse confirm(String memberId, TossPaymentConfirmRequest request) {
        String paymentKey = request.getPaymentKey();
        String orderId = request.getOrderId();

        // 1. 클레임 획득 — 경합 시 409, Redis 장애 시 503(fail-closed). 둘 다
        // TossPaymentClaim이 던지므로 여기서는 그대로 전파한다.
        tossPaymentClaim.acquire(memberId, orderId);

        // 2. 사전 점검 — Toss를 호출하기 전에 가장 흔한 실패(지갑 없음/잔액 부족)를 걸러낸다.
        // 이는 정합성 보장 수단이 아니라 최적화다: 사전 점검과 실제 차감(5단계) 사이에는
        // TOCTOU 간극이 있고, 그 간극에서 발생하는 잔액 부족은 5단계의 catch(Exception)
        // 보상 분기가 처리한다.
        //
        ...
        // 해제한다. Toss를 호출조차 하지 않았으니 이중결제 위험이 없고, 해제하지 않으면
        ...
        try {
            ...
            // amount는 DTO에서 @Digits(integer=13, fraction=0)로 이미 정수임이 보장되어
            // setScale(0, UNNECESSARY)가 예외를 던지지 않지만, 검증을 우회한 호출 경로가
            // 생기더라도 클레임이 새지 않도록 이 블록 안에 둔다.
        ...
        switch (result.getOutcome()) {
            ...
            case ALREADY_APPROVED:
                ...
                throw new BusinessException(HttpStatus.CONFLICT, "이미 처리된 결제입니다.");
            case INDETERMINATE:
                ...
                throw new BusinessException(HttpStatus.BAD_GATEWAY,
                        "결제 상태를 확인할 수 없습니다. 새 결제로 다시 시도해 주세요.");
            case SUCCESS:
            default:
                break;
        }

        // 4. 원장 기록 — 차감 금액은 요청 DTO의 amount가 아니라 Toss confirm이 확정한
        // totalAmount를 쓴다. 클라이언트가 보낸 금액을 그대로 차감하지 않는다.
        ...
        try {
            ...
            return transactionService.recordExternalPayment(command);
            ...
            // 그 외 모든 실패(사전 점검과 실제 차감 사이 TOCTOU로 인한 잔액 부족 포함) —
            // Toss는 이미 승인했으므로 cancel 보상을 시도한다. 원래 실패(e)를 보상 호출의
            // 성공/실패로 감추지 않고, 어느 쪽이든 500으로 응답한다.
     ...
     * 않는 감사 로그에만 남긴다.
     */
    private static final String CANCEL_REASON = "원장 기록 실패로 인한 자동 취소";
