...
-- created_at 단일/선두 인덱스로는 새 정렬(deadline, created_at DESC)에 순서를 제공할 수 없어
...
CREATE INDEX `idx_gp_deadline_created` ON `group_purchase` (`deadline`, `created_at` DESC);

-- 카테고리 탭은 등호 조건이라 (category, deadline, created_at) 순서로 두면 필터와 정렬을 한
-- 인덱스로 처리한다(V39의 idx_gp_category_created와 동일한 이유).
CREATE INDEX `idx_gp_category_deadline_created` ON `group_purchase` (`category`, `deadline`, `created_at` DESC);
