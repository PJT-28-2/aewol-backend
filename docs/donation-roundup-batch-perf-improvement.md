...
저장소에 일일 처리 건수에 대한 근거(시드 데이터, 문서, 이슈)가 없다. `V51__add_transaction_wallet_date_index.sql` 주석이 "잔돈 적립 배치는 `idx_txn_date`를 그대로 쓴다"고 언급하지만 이는 인덱스 선택 근거이지 건수 추정치가 아니다. 실측이 필요하면 공동구매 때처럼 시드 데이터를 직접 만들어 재현하면 되지만, 지금 이 문서는 그 실측 없이도 명백한 정합성·가용성 문제(1-1, 1-2)를 근거로 방향을 정한다(2절 참고).
