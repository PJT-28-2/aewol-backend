...
필요한 환경변수는 `S3_BUCKET`(필수), `S3_REGION`(기본 `ap-northeast-2`)이다.
멍스타그램 공개 CDN은 선택이다. `S3_PUBLIC_BASE_URL`이 비어 있으면 공개 사본을 만들지
않고 서명 URL로 사진을 보여 준다. 만료 없는 주소를 쓰려면 버킷의 `public/` prefix만
CDN이 읽도록 연 뒤 그 주소를 넣는다. 버킷 전체를 열면 비공개 일기 사진까지 노출된다.

자격증명은 설정하지 않는다 — EC2 인스턴스 프로파일(IAM Role)을 기본 제공자 체인이 찾는다.

버킷 설정:

- 퍼블릭 액세스 **전면 차단**. 조회는 기본적으로 presigned URL로만 한다
