...
- [ ] **V48/V49/V50 마이그레이션 소요 시간·기동 지연 영향**
  로컬 20만 행 기준 실측(2026-08-23, `flyway_schema_history.execution_time`): V48(FULLTEXT 인덱스) 1.4초, V49(컬럼 추가 + 백필 UPDATE) 2.8초, V50(CHECK 제약) 4.5초 — 합계 약 8.7초. V49/V50은 기존 행 전체를 훑는 연산(백필/제약 검증)이라 운영 DB 규모가 20만 행보다 크면 대략 비례해서 늘어날 걸로 예상된다. **단 V48(ngram FULLTEXT 인덱스 생성)은 이 "비례" 가정이 낙관적일 수 있다** — ngram 파서는 텍스트를 N-gram으로 쪼개 색인하는 특성상 인덱스 크기·빌드 시간이 행 수에 선형이 아니라 텍스트 조합 수에 더 가깝게 늘어나는 경우가 흔하고, 한글처럼 조합이 다양한 텍스트에서 특히 그렇다. 운영 규모가 20만 행보다 몇 배 이상 크다면 V48만 따로 운영 규모 데이터로 리허설해볼 것. Flyway가 앱 기동 시 자동 실행되는 구조라면(현재 CI는 `./gradlew flywayMigrate`를 별도 스텝으로 분리해서 돌리지만, 운영 배포 파이프라인이 어떻게 구성돼 있는지는 이 문서 범위 밖) 기동 타임아웃에 걸리지 않는지 운영 규모로 별도 확인 필요. V48(FULLTEXT 최초 생성)은 여기 더해 쓰기 잠금(Online DDL 미지원, 별도 항목 참고)도 함께 고려해야 한다.
